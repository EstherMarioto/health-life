...
<br>
  ...
  <div class = "container">
    <div class = "card painel">
      <div class = "container">
        <br>
        <h2 class="text-center"><b><i>Contato Enfermeiro</i></b></h2>
        <form name="form" method="post" action="insert/insert_enfermeiro.php">
            <div class="form-row">
              <div class="form-group col-md-8">
                  <label><b>Enfermeiros </b></label>
                    <select name="me_enfermeiro" class="form-control">
                      <?php
                        $sth = $pdo ->prepare('select * from tbl_enfermeiro');
                        $sth->execute(); 
                        foreach ($sth as $res){ 
                        extract($res);
                        echo'<option value="'. $en_codigo .'">' . $en_nome .' </option>'; 
                        } 
                      ?>
                    </select>
              </div>
