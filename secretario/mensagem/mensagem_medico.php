...
<br>
  ...
  <div class = "container">
    <div class = "card painel">
      <div class = "container">
        <br>
        <h2 class="text-center"><b><i>Contato Médico</i></b></h2>
        <form name="form" method="post" action="insert/insert_medico.php">
            <div class="form-row">
              <div class="form-group col-md-8">
                  <label><b>Médicos </b></label>
                    <select name="me_medico" class="form-control">
                      <?php
                        $sth = $pdo ->prepare('select * from tbl_medico');
                        $sth->execute(); 
                        foreach ($sth as $res){ 
                        extract($res);
                        echo'<option value="'. $me_codigo .'">' . $me_nome .'</option>'; 
                        } 
                      ?>
                    </select>
              </div>
